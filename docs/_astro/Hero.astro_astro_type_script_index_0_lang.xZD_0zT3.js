import{s as n,I as c,a as d,b as l,g as o}from"./hero-finder.CjyutB_r.js";let t=c;const a={tiles:document.getElementById("finder-tiles"),intermediateAddress:document.getElementById("finder-intermediate-address"),intermediateCoworking:document.getElementById("finder-intermediate-coworking"),result:document.getElementById("finder-result")},u=document.getElementById("finder-result-card");function s(e){if(Object.values(a).forEach(r=>r.classList.add("finder__stage--hidden")),e.stage==="tiles"){a.tiles.classList.remove("finder__stage--hidden");return}if(e.stage==="intermediate"){e.tileId==="address"&&a.intermediateAddress.classList.remove("finder__stage--hidden"),e.tileId==="coworking"&&a.intermediateCoworking.classList.remove("finder__stage--hidden");return}e.stage==="result"&&(a.result.classList.remove("finder__stage--hidden"),m(e.packageId))}function m(e){const r=o(e);u.innerHTML=`
      ...
      </div>`}document.querySelectorAll("[data-tile]").forEach(e=>{e.addEventListener("click",()=>{t=n(e.dataset.tile),s(t)})});a.intermediateAddress.querySelectorAll("[data-option]").forEach(e=>{e.addEventListener("click",()=>{t.stage==="intermediate"&&(t=d(t,e.dataset.option),s(t))})});a.intermediateCoworking.querySelectorAll("[data-option]").forEach(e=>{e.addEventListener("click",()=>{t.stage==="intermediate"&&(t=d(t,e.dataset.option),s(t))})});["back-from-address","back-from-coworking","back-from-result"].forEach(e=>{document.getElementById(e)?.addEventListener("click",()=>{t=l(),s(t)})});s(t);
